"""
财务审计意见下载 fina_audit
说明：无 VIP 接口，按 ts_code 遍历初始下载，增量按 ann_date 区间拉取。
"""
import sys
sys.path.append('.')
...
from orm_models.table_models import FinaAudit, StockBasic, UpdateRecord


class FinaAuditTushare:
    TABLE = FinaAudit
    ...
    def pull_initial(self):
        """ 全量下载：遍历所有 ts_code，仅年报（period 以 1231 结尾） """
        table_name = self.TABLE.__tablename__
    ...
    def pull_incremental(self, days=30):
        ...
        while True:
            try:
                ...
                loguru_logger.warning(f'{table_name} 增量等待20秒重试: {e}')
                time.sleep(20)
