...
class DetailDataBase:
    ...
    def get_data_from_tushare(self, date, **kwargs):
        if not self.limit:
            ...
            df = self.from_api(trade_date=date.strftime('%Y%m%d'), limit=self.limit, offset=offset, fields=self.fields)
        return df

    def get_data_from_tushare_wait(self, date, **kwargs):
        # 如果接口有限制则等待10秒，直到可以继续调用
        while True:
            try:
                df = self.get_data_from_tushare(date, **kwargs)
                break
            except Exception as e:
                self.logger.info(f'在下载表{self.to_table.__tablename__}的{date}时等待10秒。error:{e}')
        return df

    def get_data_from_tushare_unlimit(self, date):
        df = pd.DataFrame()  # 空DataFrame
        loop_num = 0 # 循环次数
        if not self.limit:  # 如果limit=0，则没有限制，直接下载
            df = self.get_data_from_tushare_wait(date)  # 取数
        else:  # 如果limit=1，则有限制，循环下载
            data_length = self.limit # 为了取第一次数，data_length默认设置为limit
            # 如果返回的数据长度大于等于limit（一般不会大于limit），说明继续遍历。
            while data_length >= self.limit:
                df2 = self.get_data_from_tushare_wait(date, offset=loop_num*self.limit) #  循环取数，第loop_num个DataFrame(limit行)
                data_length = len(df2) # 数据长度
                df = pd.concat([df, df2], axis=0) # 合并df
                loop_num += 1 # 将循环次数加1
            df = df.sort_values(by='trade_date') # 按照trade_date排序
            df = df.reset_index(drop=True)  # 将排序前的序号删掉
        return df
    ...
    def process_data(self, df):
        ...
        if len(df) > 0:
            ...
            if self.update_flag:
                df = df.sort_values(by=['ts_code', 'update_flag'], ascending=[True, False])
    ...
    def update_data(self):
        ...
        for date in self.update_date_list:               # 遍历更新日期列表
            df = self.get_data_from_tushare_unlimit(date)   # 下载数据（从数据库最后日期到今天的
            if len(df) == 0:
                continue # 如果没有数据，则没有更新日期
            ...
            with engine.begin() as con:
                ...
                df_record = pd.DataFrame([[date]], columns=['data_datetime'])
                df_record['table_name'] = self.to_table.__tablename__
                df_record['created_datetime'] = self.today
                df_record['table_name_data_datetime'] = df_record.apply(lambda x: x['table_name'] + str(x['data_datetime']), axis=1)
                df_record.to_sql(UpdateRecord.__tablename__, con=con, if_exists='append', index=False) # 记录更新

            percent = round((self.update_date_list.index(date) + 1) / len(self.update_date_list) * 100, 2)
            if  percent - old_percent >= 3:
