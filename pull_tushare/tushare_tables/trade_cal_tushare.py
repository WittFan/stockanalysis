...
class TradeCalTushare(MetaDataBase):
    ...
    def down(self):
        ...
        for i in ['SSE', 'SZSE', 'CFFEX', 'SHFE', 'CZCE', 'DCE', 'INE']:
            df_table2 = pro.trade_cal(exchange=i)
    ...
    def record(self, today, df_table):
        data = [today_todatetime(), self.to_table.__tablename__, today_todatetime(), str(self.to_table.__tablename__) + str(today_todatetime())]
        df_record = pd.DataFrame([data], columns=['data_datetime', 'table_name', 'created_datetime', 'table_name_data_datetime'])
    ...
    def pull(self):
        ...
        if today > datetime.datetime(year=date.year, month=12, day=31):
