...
class IndexMemberTushare(MetaDataBase):
    def __init__(self):
        super().__init__()
        self.from_api = pro.index_member
    ...
    def down(self):
        ...
        df = pd.DataFrame(['000016.SH'], columns=['index_code'])
        index_codes = pd.concat([index_codes, df], axis=0, ignore_index=True)
        ...
        df['index_code_con_code_in_date_out_date'] = df.apply(lambda x: x['index_code'] + x['con_code'] + str(x['in_date']) +str(x['out_date']), axis=1)
        return df


if __name__ == "__main__":
    # IndexMemberTushare().down()
