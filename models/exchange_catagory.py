...
class TradeWeek(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    exchange = Column(String, index=True, comment='交易所 SSE上交所 SZSE深交所')
    cal_date = Column(String, index=True, comment='每周交易日期')
    pretrade_date = Column(String, comment='上一个周最后一个交易日')

class TradeMonth(Base):
    __tablename__ = "trade_month"
    id = Column(Integer, primary_key=True, autoincrement=True)
    exchange = Column(String, index=True, comment='交易所 SSE上交所 SZSE深交所')
    cal_date = Column(String, index=True, comment='每月交易日期')
    pretrade_date = Column(String, comment='上一个月最后一个交易日')