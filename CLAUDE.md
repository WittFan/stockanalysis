...
- `orm_models/` - 数据库 ORM 层，DataApi（api.py）是统一数据接口
- `pull_tushare/` - Tushare 数据拉取（多线程，支持增量更新）
- `datafeed/` - 数据加载（Duckdbloader） + 表达式因子计算引擎（expr.py）
- `config.py` - 全局配置（DB_URI、tushare_token）
...
- 因子函数定义在 `datafeed/expr_funcs*.py`，签名为 `func(pd.Series, *args) -> pd.Series`
